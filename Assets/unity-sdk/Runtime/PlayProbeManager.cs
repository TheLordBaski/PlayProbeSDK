...
namespace PlayProbe
{
    ...
    public class PlayProbeManager : MonoBehaviour
    {
        ...
        private const string API_BASE = "https://app.playprobe.io/";
        ...
        private void Awake()
        {
            try
            {
                ...
                BuildRuntimeConfig();
                PlayProbeHttp.Configure(_runtimeConfig);

                Events = new PlayProbeEvents(_runtimeConfig);
                Survey = new PlayProbeSurvey(_runtimeConfig, Events);
        ...
        private IEnumerator StartSessionAsync()
        {
            ...
            try
            {
                if (Survey != null)
                {
                    surveySchema = Survey.GetRegisteredSchema();
                }
            }
            catch (Exception ex)
            {
        ...
        private PlayProbeSdkSessionEndRequest BuildSessionEndPayload()
        {
            ...
            try
            {
                if (Survey != null)
                {
                    surveyResponses = Survey.GetBufferedResponses();
                }
            }
            catch (Exception ex)
            {
        ...
        private bool TryApplyStartSessionResponse(string responseJson, out string error)
        {
            ...
            try
            {
                ...
                try
                {
                    Survey?.LoadTriggers(surveyTriggers);
                }
                catch (Exception ex)
                {
