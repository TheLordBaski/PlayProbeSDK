...
namespace PlayProbe
{
    ...
    {
        private readonly SurveyRegistration _registration;

        internal SurveyBuilder(SurveyRegistration registration)
        {
        ...
        {
            return AddQuestion(label, sdkQuestionId, "yes_no", new[] { "Yes", "No" }, required);
        }
    ...
    public class PlayProbeSurvey
    {
        ...
        private readonly List<SurveyRegistration> _registrations = new List<SurveyRegistration>();
        ...
        public PlayProbeSurvey(PlayProbeConfig config, PlayProbeEvents events)
        {
            _config = config;
            _events = events;
        }
        ...
        public SurveyBuilder Register(string triggerKey)
        {
            ...
            SurveyRegistration registration = FindOrCreateRegistration(resolvedTriggerKey);
        ...
        public List<SurveySchemaItem> GetRegisteredSchema()
        {
            ...
            {
                SurveyRegistration registration = _registrations[registrationIndex];
        ...
        private SurveyRegistration FindOrCreateRegistration(string triggerKey)
        {
            for (int index = 0; index < _registrations.Count; index++)
            {
                SurveyRegistration registration = _registrations[index];
            ...
            SurveyRegistration created = new SurveyRegistration
            {
                trigger_key = triggerKey
            };
        ...
        private void SubmitSurveyImmediately(string triggerKey, List<SurveyResponse> responses)
        {
            ...
            try
            {
                ...
                manager.StartCoroutine(PostSurveySubmit(payloadJson));
            }
            catch (Exception ex)
            {
        ...
        private bool ResolvePauseDuringSurveyEnabled()
        {
            ...
            if (manager != null)
            {
                if (TryReadBooleanMember(manager, "pauseGameDuringSurvey", out bool pauseGameDuringSurvey))
                {
                    return pauseGameDuringSurvey;
                }

                if (TryReadBooleanMember(manager, "pauseTimeDuringSurvey", out bool pauseTimeDuringSurvey))
                {
